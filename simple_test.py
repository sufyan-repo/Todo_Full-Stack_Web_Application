...
def test_application():
    ...
    try:
        ...
        if response.status_code == 200:
            data = response.json()
            if "response" in data:
                print("   OK API connectivity works")
            else:
        ...
        print(f"   FAIL API connectivity error: {e}")
        return False
    ...
    try:
        ...
        if response.status_code == 200:
            ...
            if "response" in data and "Successfully added task" in data["response"]:
                print("   OK Task addition works")
            else:
                print("   FAIL Task addition failed")
                return False
        else:
            print(f"   FAIL Task addition request failed with status {response.status_code}")
            return False
    except Exception as e:
        print(f"   FAIL Task addition error: {e}")
        return False
