...
export default function DashboardPage() {
  ...
  React.useEffect(() => {
    ...
    const fetchTodos = async () => {
      if (typeof window !== 'undefined') {
        ...
        try {
          ...
          // If fetch fails due to auth error, redirect to login
          if (error instanceof Error && (error.message.includes('Authentication') || error.message.includes('401'))) {
            localStorage.removeItem('token');
            localStorage.removeItem('user_name');
            router.replace('/auth/sign-in');
          } else if (error instanceof Error && error.message.includes('Failed to fetch')) {
            // For network errors, show toast but don't redirect
            addToast({
              type: 'error',
              message: 'Unable to connect to the server. This might be due to browser extensions. Try disabling ad blockers or privacy tools.',
            });
          } else {
    ...
    // Handle the todo fetch and cleanup
    ...
    if (typeof window !== 'undefined') {
      ...
      if (checkAuthAndRedirect()) {
        // Use a slight delay to ensure ProtectedRoute has had a chance to check auth
        timer = setTimeout(fetchTodos, 100);
      } else {
        // If not authenticated, still set loading to false
        setLoading(false);
      }
      ...
      // On server side, don't run this effect
      setLoading(false);
    }
  }, [addToast, router]);
  ...
  return (
    <ProtectedRoute>
      <div className="min-h-[calc(100vh-80px)] bg-background">
              ...
              <div className="relative z-10">
                ...
                <p className="text-sm opacity-60 font-bold max-w-md">
                  {totalCount === 0
                    ...
                    : `Completed ${completedCount} tasks. ${totalCount - completedCount} tasks remaining.`}
                </p>
              </div>
            </div>
          ...
          {!loading && todos.length === 0 && (
            <div className="animate-welcome" style={{ animationDelay: '0.3s' }}>
              <EmptyState
                type="no-todos"
                title="No Tasks Yet"
                description="Get started by adding your first task to your list."
                cta={{
        ...
        <ChatBot userId={userName || 'default_user'} />
      </div>
    </ProtectedRoute>
  );
}
